...
O Nimbus e os supervisors, armazenam seus estados no Zookeeper, podendo ser reinicializados sem perdê-los em caso de falha \cite{LearningStorm}. Caso algum supervisor falhe, seus workers são reatribuídos pelo Nimbus a outro supervisor, no entanto, ficando impedido de receberem novas tuplas \cite{StormPython}.

Por, o Nimbus, dentre suas atribuições, é responsável também por reinicializar as tasks caso uma delas venha a falhar, e se o mesmo acontecer com ele próprio, as tasks em execução não são afetadas, mas novas topologias são impedidas de serem submetidas ao cluster \cite{LearningStorm}, assim como reatribuições \cite{StormPython}.

Como é possível perceber, o Storm utiliza o método de recuperação por replicação, reatribuindo determinado processamento a outro recurso. Nessa metodologia, são necessário no mínimo dois recursos de hardware, os quais não são suficientes para contingência caso ambos falhem \cite{DStream3}. Sendo assim, o modelo de recuperação de falhas do Spark atende melhor ao requisito de tolerância a falhas para aplicações que o priorize, com as desenvolvidas nessa monografia, tanto para o processamento em batch quanto para o de stream de tweets.
